...
export class NetworkLayoutEngine {
   ...
   */
  constructor(relationships = [], config = {}) {
  ...
  _extractPathogens() {
    ...
    return Array.from(pathogenIds)
      .map(id => simplePathogens.find(p => p.id === id))
  ...
  forceDirectedLayout(options = {}) {
    ...
    const nodes = this.pathogens.map((pathogen, i) => ({
      id: pathogen.id,
      name: pathogen.name,
      ...pathogen,
  ...
  validateLayout(layout) {
    ...
    if (!layout?.nodes || !Array.isArray(layout.nodes)) {
      return { valid: false, issues: ['No nodes in layout'] };
    }
  ...
  getPerformanceMetrics() {
    return {
      ...this.performanceMetrics,
      summary: {
        allLayouts: Object.values(this.performanceMetrics),
        averageTime: Object.values(this.performanceMetrics).reduce((a, b) => a + b, 0) /
 ...
 */
export const createLayoutEngine = (relationships, width = 1000, height = 800) => {
