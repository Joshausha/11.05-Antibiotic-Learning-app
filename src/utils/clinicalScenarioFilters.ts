...
/**
 * Filter criteria for antibiotic coverage
 */
...
/**
 * Northwestern spectrum coverage for pathogens
 */
...
/**
 * Additional filtering criteria for a scenario
 */
...
interface RiskFactorGroup {
  additionalFilters?: { [key: string]: FilterCriteria };
  criteria?: string[];
  [key: string]: unknown;
}
...
/**
 * Valve type configuration for endocarditis
 */
...
/**
 * Standard scenario with severity modifiers
 */
interface StandardScenario extends ClinicalScenario {
  severityModifiers?: { [key: string]: SeverityLevelConfig };
  riskFactors?: { [key: string]: RiskFactorGroup };
  severityLevels?: { [key: string]: SeverityLevelConfig };
  patientGroups?: { [key: string]: PatientGroupConfig };
  valveTypes?: { [key: string]: ValveTypeConfig };
  ageGroups?: { [key: string]: AgeGroupConfig };
  procedures?: string[];
  timing?: string;
  ageGroup?: string;
}
...
/**
 * Result from applying a clinical scenario
 */
interface ScenarioResult {
  ...
  totalCount: number;
  appliedModifiers?: { [key: string]: unknown };
  clinicalGuidance?: ClinicalGuidance;
  evidenceBasis?: string;
  performance?: {
...
/**
 * Antibiotic with priority score
 */
interface PrioritizedAntibiotic extends Antibiotic {
  priorityScore?: number;
}

/**
 * Clinical recommendations structure
 */
interface Recommendations {
  firstLine: PrioritizedAntibiotic[];
  secondLine: PrioritizedAntibiotic[];
  avoid: Antibiotic[];
...
/**
 * Full clinical decision support result
 */
interface ClinicalDecisionResult extends ScenarioResult {
 ...
 */
const EMERGENCY_SCENARIOS: { [key: string]: StandardScenario } = {
  septicShock: {
    name: 'Septic Shock',
    urgency: 'critical',
 ...
 */
const STANDARD_SCENARIOS: { [key: string]: StandardScenario } = {
  communityPneumonia: {
  ...
  intraabdominal: {
    ...
    evidenceBasis: 'IDSA Intra-abdominal Infection Guidelines'
  },
  endocarditis: {
    ...
    evidenceBasis: 'IDSA Endocarditis Guidelines'
  }
};
 ...
 */
export const SURGICAL_PROPHYLAXIS: { [key: string]: StandardScenario } = {
  cleanWound: {
    ...
    evidenceBasis: 'IDSA Surgical Prophylaxis Guidelines'
  },
  cleanContaminated: {
 ...
 */
export const PEDIATRIC_SCENARIOS: { [key: string]: StandardScenario } = {
  febrileNeutropenia: {
    ...
    evidenceBasis: 'IDSA Fever and Neutropenia Guidelines'
  },
  neonatalSepsis: {
    ...
    evidenceBasis: 'AAP/IDSA Neonatal Sepsis Guidelines'
  }
};
 ...
 */
export function applyClinicalScenario(
  ...
  let scenario: StandardScenario | undefined =
    STANDARD_SCENARIOS[scenarioKey] ||
  ...
  if (modifiers.severity && scenario.severityLevels?.[String(modifiers.severity)]) {
    const severityConfig = scenario.severityLevels[String(modifiers.severity)];
  ...
  const results = antibiotics.filter(antibiotic => {
    ...
    const meetsNorthwesternCriteria = Object.entries(effectiveFilters).every(
      ([category, criteria]) => {
        if (!criteria.enabled) return true;
        const coverage = (antibiotic.northwesternSpectrum?.[category] as number) || 0;
    ...
    if (scenario.additionalCriteria) {
      const { routeFilter, cellWallActiveOnly } = scenario.additionalCriteria;

      if (routeFilter && !routeFilter.includes(String(antibiotic.routeColor))) {
        return false;
      }
...
export function getRecommendedScenarios(
  ...
  patientContext: { [key: string]: unknown } = {}
): Recommendation[] {
  ...
  if (context.includes('pneumonia')) {
    const key =
      context.includes('hospital') || context.includes('nosocomial')
        ? 'hospitalPneumonia'
        : 'communityPneumonia';
    recommendations.push({
      key,
  ...
  if (context.includes('uti') || context.includes('urinary')) {
    const key =
      context.includes('complicated') || context.includes('pyelonephritis')
        ? 'complicatedUTI'
        : 'uncomplicatedUTI';
    recommendations.push({
      key,
 ...
 */
export function generateClinicalDecisionSupport(scenarioResults: ScenarioResult): ClinicalDecisionResult {
  const { scenario, matchingAntibiotics } = scenarioResults;
  ...
  if (!scenario) {
    return {
      ...
      recommendations: {
        ...
        secondLine: [],
        avoid: [],
        considerations: ['Unable to generate recommendations - scenario not found']
      },
      decisionSupport: {
        evidence: '',
        contraindications: [],
  ...
  const prioritizedAntibiotics: PrioritizedAntibiotic[] = matchingAntibiotics
    .map(antibiotic => {
      let score = 0;
      ...
      if (scenario.additionalCriteria?.routeFilter?.includes(String(antibiotic.routeColor))) {
        score += 5;
      }
  ...
  if (scenario.clinicalGuidance) {
    if (scenario.clinicalGuidance.dosing) {
      recommendations.considerations.push(`Dosing: ${scenario.clinicalGuidance.dosing}`);
    }
    if (scenario.clinicalGuidance.duration) {
      recommendations.considerations.push(`Duration: ${scenario.clinicalGuidance.duration}`);
    }
    if (scenario.clinicalGuidance.monitoring) {
      recommendations.considerations.push(`Monitoring: ${scenario.clinicalGuidance.monitoring}`);
    }
  }

  if (scenario.emergencyProtocol) {
    recommendations.considerations.unshift(`🚨 ${scenario.emergencyProtocol.clinicalAlert}`);
  }
 ...
 */
export function validateClinicalScenario(
  scenarioKey: string,
  customScenario: StandardScenario | null = null
): ValidationResult {
  ...
  if (
    scenario.additionalCriteria?.cellWallActiveOnly &&
