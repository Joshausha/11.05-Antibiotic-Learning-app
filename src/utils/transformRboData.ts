...
/**
 * Type definitions for RBO data transformation
 */

interface EmpiricalTherapy {
  condition: string;
  therapy: string;
}

interface RboConditionData {
  id: string;
  category: string;
  name: string;
  description: string;
  commonPathogens: string[];
  empiricAntibioticTherapy: EmpiricalTherapy[];
...
const rboJsonData: RboConditionData[] = [
  {
 ...
 * @param transformFn - Optional transform function for testing
 ...
 */
export const generateMedicalConditionsFile = (transformedData: RboConditionData[]): string => {
  const fileHeader = `/**
