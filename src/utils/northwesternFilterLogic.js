...
export const NORTHWESTERN_CATEGORIES = {
  MRSA: { 
    name: 'MRSA', 
 ...
 */
export function applyNorthwesternFilters(antibiotics, filterState) {
  ...
  if (filterState.textSearch) {
    ...
    results = results.filter(antibiotic => 
      ...
      antibiotic.class.toLowerCase().includes(searchTerm) ||
 ...
 */
export function applyCombinationFilters(antibiotics, combinations) {
  const startTime = performance.now();
 ...
 */
export function getFilterSuggestions(antibiotics, clinicalContext = '') {
  const suggestions = [];
  ...
  antibiotics.forEach(antibiotic => {
    if (antibiotic.northwesternSpectrum) {
      Object.entries(antibiotic.northwesternSpectrum).forEach(([category, coverage]) => {
        ...
        categoryStats[category][coverage]++;
      });
    }
  });
 ...
 */
export function getFilterPerformanceReport() {
  return {
 ...
 */
export function validateFilterConfiguration(filterState) {
  const warnings = [];
  const errors = [];
 ...
 */
export function exportFilterState(filterState) {
  return {
    version: '1.0.0',
    timestamp: Date.now(),
    state: {
 ...
 */
export function importFilterState(exportedState) {
