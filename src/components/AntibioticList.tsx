...
interface EnhancedAntibiotic extends Antibiotic {
  duration?: string | null;
  durationContext?: any | null;
}

interface AntibioticListProps {
  pathogen?: Pathogen | null;
  antibiotics?: Antibiotic[];
  onSelectAntibiotic?: (antibiotic: EnhancedAntibiotic) => void;
  selectedAntibiotic?: EnhancedAntibiotic | null;
}

const AntibioticList: React.FC<AntibioticListProps> = ({
  pathogen = null,
  antibiotics = [],
  onSelectAntibiotic = () => {},
  ...
  const enhancedAntibiotics = useMemo((): EnhancedAntibiotic[] => {
    if (!pathogen || !antibiotics) return [];

    const pathogenKey = pathogen.id?.toString();
    const enhancedData = enhancedPathogenAntibioticMap[pathogenKey as keyof typeof enhancedPathogenAntibioticMap];

    if (!enhancedData) return antibiotics as EnhancedAntibiotic[];
    ...
    return antibiotics.map(antibiotic => {
      const enhanced = enhancedData.antibiotics.find(
        (enh: any) => enh.antibioticId === antibiotic.id
      );
  ...
  const getRouteIcon = (route?: string): React.ReactNode => {
  ...
  const getMechanismIcon = (mechanism?: string): React.ReactNode => {
  ...
  const getInfectionIcon = (commonUses?: string[]): React.ReactNode => {
  ...
  const groupedAntibiotics = enhancedAntibiotics.reduce((groups: Record<string, EnhancedAntibiotic[]>, antibiotic) => {
    const effectiveness = (antibiotic as any).effectiveness || 'unknown';
    ...
    <div className="bg-white rounded-lg shadow-sm border" data-testid="antibiotic-list">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {(() => {
                            const fullAntibiotic = getAntibioticById(antibiotic.id);
      ...
      <div className="px-4 py-3 bg-gray-50 border-t rounded-b-lg">
        <div className="flex justify-between items-center text-xs text-gray-500">
          ...
          <span>
            Effective: {enhancedAntibiotics.filter((ab) => (ab as any).effectiveness === 'high' || (ab as any).effectiveness === 'medium').length}
          </span>
          <span>
