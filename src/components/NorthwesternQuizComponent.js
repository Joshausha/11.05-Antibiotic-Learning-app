...
const NorthwesternQuizComponent = ({ 
  question, 
  currentAnswer, 
  onAnswerSelect, 
  showResult = false,
  ...
  const relevantAntibiotic = useMemo(() => {
    ...
    const antibioticNames = [
      'vancomycin', 'ceftriaxone', 'azithromycin', 'ciprofloxacin',
      'piperacillin-tazobactam', 'piperacillin', 'meropenem', 'ceftazidime',
      'linezolid', 'daptomycin', 'penicillin', 'amoxicillin', 'cefazolin'
    ];
  ...
  const handleSegmentHover = useCallback((segmentKey, event, context) => {
    setSelectedSegment(segmentKey);
  }, []);

  const handleSegmentClick = useCallback((segmentKey, event, context) => {
  ...
  const renderVisualComponent = () => {
    ...
    return (
      <div className="my-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
        ...
        <div className="flex justify-center mb-4">
          <NorthwesternPieChart
            ...
            selectedSegments={selectedSegment ? [selectedSegment] : 
              (question.targetSegment && question.targetSegment !== 'multiple' ? [question.targetSegment] : [])}
        ...
        {question.targetSegment && question.targetSegment !== 'multiple' && question.targetSegment !== 'visual' && !showResult && (
