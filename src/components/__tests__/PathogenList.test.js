...
describe('PathogenList - Medical Pathogen Enumeration & Clinical Accuracy', () => {
  ...
  describe('Medical Data Display & Accuracy', () => {
    ...
    test('applies correct gram status visual indicators', () => {
      ...
      // Gram positive should have purple styling
      ...
      // Gram negative should have red styling  
      const gramNegativeElements = screen.getAllByText(/gram.*negative/i);
    ...
    test('displays severity levels with appropriate clinical color coding', () => {
      ...
      const highSeverityElements = screen.getAllByText(/high.*severity/i);
      ...
      const mediumSeverityElements = screen.getAllByText(/medium.*severity/i);
      mediumSeverityElements.forEach(element => {
        expect(element).toHaveClass('text-yellow-600');
        expect(element).toHaveClass('bg-yellow-100');
      });
      
      // Low severity should be green
      const lowSeverityElements = screen.getAllByText(/low.*severity/i);
    ...
    test('maps infection sites to appropriate clinical icons', () => {
      ...
      // CNS infections should show brain icon
      const cnsPathogen = screen.getByText('Streptococcus pneumoniae');
      const cnsContainer = cnsPathogen.closest('.pathogen-card');
      expect(cnsContainer).toContainHTML('CNS'); // Brain icon label
      
      // Respiratory infections should show lung icon
      expect(cnsContainer).toContainHTML('Respiratory'); // Lung icon label
      
      // Cardiovascular infections should show heart icon
      expect(cnsContainer).toContainHTML('Cardiovascular'); // Heart icon label
    });

    test('handles complex multi-site infection patterns', () => {
      render(<PathogenList {...defaultProps} />);
      
      // MRSA with multiple infection sites
      const mrsaElement = screen.getByText('Methicillin-resistant Staphylococcus aureus');
      const mrsaContainer = mrsaElement.closest('.pathogen-card');
      
      // Should show up to 3 most relevant icons
      expect(mrsaContainer).toContainHTML('Skin/Soft tissue');
      expect(mrsaContainer).toContainHTML('Cardiovascular');
      expect(mrsaContainer).toContainHTML('Bone/Joint');
    });
  });
