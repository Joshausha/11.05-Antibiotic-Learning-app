...
import { DurationIndicatorProps } from '../types/component.types';
    ...
    if (parsedDuration.type === 'age-based' && parsedDuration.ageGroups) {
      return (
        <div className="space-y-1">
          ...
          {parsedDuration.ageGroups.map((group: any, index: number) => (
 ...
 */
export const DurationLegend: React.FC<{ className?: string }> = ({
  className = ''
}) => {
  const legendItems = [
    {
 ...
 */
export const DurationSummary: React.FC<{
  durations?: Array<{
    duration: string;
