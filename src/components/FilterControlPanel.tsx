...
interface QuickFilter {
  ...
  type: 'emergency' | 'recommended' | 'standard';
}

interface FilterControlPanelProps {
  filterState: FilterState;
  onFilterChange: (newState: FilterState) => void;
  onScenarioApply?: (scenarioKey: string) => void;
  onFilterReset: () => void;
  screenSize?: 'mobile' | 'tablet' | 'desktop';
  emergencyMode?: boolean;
  showAdvancedControls?: boolean;
  className?: string;
  enableFilterHistory?: boolean;
...
const FilterControlPanel: React.FC<FilterControlPanelProps> = ({
  ...
  // Component state
  const [activePanel, setActivePanel] = useState<'categories' | 'scenarios' | 'advanced'>('categories');
  ...
  const generateFilterDescription = useCallback((state: FilterState): string => {
    const parts: string[] = [];

    if (state.clinicalScenario) {
      parts.push(CLINICAL_SCENARIOS?.[state.clinicalScenario as keyof typeof CLINICAL_SCENARIOS]?.name || state.clinicalScenario);
    }
    ...
    if (state.routeFilter?.length ?? 0 > 0) {
  ...
  const handleCoverageChange = useCallback((category: string, coverage: number) => {
    const newCategories = { ...filterState.northwesternCategories };
  ...
  const handleRouteToggle = useCallback((route: string) => {
  ...
  const handleScenarioClick = useCallback((scenarioKey: string) => {
    const newState = new FilterState();
  ...
  // Handle filter save
  const handleFilterSave = useCallback((name: string) => {
  ...
  const panelConfig = useMemo((): PanelConfig => {
    const baseConfig: Record<string, PanelConfig> = {
      mobile: {
        panelHeight: '60vh',
    ...
    return { ...(baseConfig[screenSize] || baseConfig.desktop), ...(emergencyOptimizations as any) };
  ...
  const activeCategories = useMemo(() => {
    return Object.entries(filterState.northwesternCategories || {})
      ...
      .map(([category, config]) => ({
        category,
        ...config,
        info: NORTHWESTERN_CATEGORIES?.[category as keyof typeof NORTHWESTERN_CATEGORIES]
      }));
  ...
  const quickFilters = useMemo((): QuickFilter[] => {
    ...
    return [
      ...emergency.map(([key, scenario]) => ({ key, scenario, type: 'emergency' as const })),
      ...recommended.map(({ key, scenario }: any) => ({ key, scenario, type: 'recommended' as const })),
      ...standard.map(([key, scenario]) => ({ key, scenario, type: 'standard' as const }))
                  ...
                  {isActive && (
                    <div className="coverage-slider-container">
                      <label className="coverage-slider-label">
                        Min Coverage: {COVERAGE_LEVELS?.[minCoverage as keyof typeof COVERAGE_LEVELS]?.name}
                      </label>
                      <input
                        type="range"
                        min="0"
                        ...
                        step="1"
                        value={minCoverage}
                        onChange={(e) => handleCoverageChange(category, parseInt(e.target.value, 10))}
                      ...
                      <div className="coverage-levels">
                        {[0, 1, 2].map(level => (
                          ...
                          >
                            {COVERAGE_LEVELS?.[level as keyof typeof COVERAGE_LEVELS]?.name}
                          </span>
                        ))}
                      </div>
                    </div>
          ...
          {recommendedScenarios.length > 0 && (
            <div className="recommended-scenarios">
              ...
              <div className="recommended-scenarios-list">
                {recommendedScenarios.slice(0, 3).map(({ key, scenario, reason }: any) => (
                  <button
                    key={key}
          ...
          {enableFilterHistory && filterHistory.length > 0 && (
            <div className="filter-history-section">
              ...
              <div className="filter-history-list">
                {filterHistory.slice(0, 5).map((item) => (
                  <button
                    key={item.timestamp}
