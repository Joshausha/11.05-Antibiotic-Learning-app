/**
 * Northwestern Interaction Demo Component (TypeScript)
 *
...
interface DemoMode {
  id: 'integrated' | 'standalone' | 'comparison';
  label: string;
  description: string;
}
 ...
 */
const DEMO_ANTIBIOTIC: DemoAntibiotic = {
  id: 'demo-vancomycin',
  name: 'Vancomycin',
  class: 'Glycopeptide',
...
interface ControlPanelProps {
  ...
  onEmergencyModeToggle: () => void;
  showTooltips: boolean;
  onShowTooltipsToggle: () => void;
  showDetailPanel: boolean;
  onShowDetailPanelToggle: () => void;
}
  ...
  <div className="control-panel bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <p className="text-xs text-gray-600 mt-1">Adjusts complexity and content detail</p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-600 mt-1">Prioritizes critical clinical information</p>
      </div>
      ...
      <div>
        <label className="flex items-center space-x-3">
          ...
          />
          <span className="text-sm font-medium text-gray-700">Rich Tooltips</span>
        </label>
        <p className="text-xs text-gray-600 mt-1">Show enhanced clinical context on hover</p>
      </div>
      ...
      <div>
        <label className="flex items-center space-x-3">
          ...
          />
          <span className="text-sm font-medium text-gray-700">Detail Panel</span>
        </label>
        <p className="text-xs text-gray-600 mt-1">Show advanced learning features</p>
      </div>
    </div>
  </div>
);
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <div className="space-y-1 text-sm">
          <div>
            ...
            <span className="text-blue-800 ml-2 font-medium">{performanceMetrics.averageResponseTime.toFixed(1)}ms</span>
          </div>
          <div>
            <span className="text-blue-600">Interactions:</span>
            <span className="text-blue-800 ml-2 font-medium">{performanceMetrics.interactions}</span>
          </div>
          {performanceMetrics.slowInteractions > 0 && (
            <div>
              <span className="text-red-600">Slow Responses:</span>
              <span className="text-red-700 ml-2 font-medium">{performanceMetrics.slowInteractions}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1 text-sm">
          ...
          <div>
            <span className="text-blue-600">Interactions:</span>
            <span className="text-blue-800 ml-2 font-medium">{learningSession.interactions}</span>
          </div>
          <div>
            <span className="text-blue-600">Insights:</span>
            <span className="text-blue-800 ml-2 font-medium">{learningSession.insights.length}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1 text-sm">
          <div>
            ...
            <span className="text-blue-800 ml-2 font-medium">{selectedSegments.size} segments</span>
          </div>
          {selectedSegments.size > 0 && (
            <div className="mt-2">
              {Array.from(selectedSegments).map((segment) => (
                <span key={segment} className="inline-block px-2 py-1 bg-blue-200 text-blue-800 text-xs rounded mr-1 mb-1">
                  {segment.replace(/_/g, ' ')}
                </span>
              ))}
            </div>
 ...
 */
const NorthwesternInteractionDemo: FC = () => {
  // Demo control state
  const [educationLevel, setEducationLevel] = useState<EducationLevel>('resident');
  const [emergencyMode, setEmergencyMode] = useState<boolean>(false);
  const [showTooltips, setShowTooltips] = useState<boolean>(true);
  const [showDetailPanel, setShowDetailPanel] = useState<boolean>(false);
  const [demoMode, setDemoMode] = useState<'integrated' | 'standalone' | 'comparison'>('integrated');

  // Use the interaction hook for state management
  const { hoveredSegment, selectedSegments, tooltipData, performanceMetrics, learningSession, handlers, utils } =
  ...
  const handleClinicalInsight = (insight: any): void => {
    console.log('Clinical Insight Generated:', insight);
  };

  // Handle learning progress
  const handleLearningProgress = (progress: any): void => {
  ...
  return (
    <div className="northwestern-interaction-demo max-w-6xl mx-auto p-6">
      ...
      <div className="demo-header mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Northwestern Pie Chart - Enhanced Interactions Demo</h1>
          ...
          {modes.map((mode) => (
            <button
              ...
              className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
                demoMode === mode.id ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
              }`}
            >
              {mode.label}
            </button>
        ...
        <p className="text-sm text-gray-600">{modes.find((m) => m.id === demoMode)?.description}</p>
      </div>
      ...
      <div className="demo-content">
        ...
        {demoMode === 'integrated' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="chart-section">
              <h2 className="text-xl font-bold text-gray-900 mb-4">{DEMO_ANTIBIOTIC.name} Coverage Analysis</h2>
            ...
            {showDetailPanel && selectedSegments.size > 0 && (
              <div className="detail-section">
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Selected Segments Details</h3>
                  <div className="space-y-4">
                    {Array.from(selectedSegments).map((segment) => (
                      <div key={segment} className="border-b pb-4 last:border-b-0">
                        <h4 className="font-medium text-gray-800 mb-2">{segment.replace(/_/g, ' ')}</h4>
                        <p className="text-sm text-gray-600">
                          Clinical information for {segment.replace(/_/g, ' ')} would be displayed here.
                        </p>
                      </div>
                    ))}
                  </div>
                  <button
                    onClick={() => utils?.clearSelections?.()}
                    className="w-full mt-4 px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded text-sm font-medium"
                  >
                    Clear Selection
                  </button>
                </div>
        ...
        {demoMode === 'comparison' && (
          <div className="comparison-demo">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Chart Comparison View</h2>
      ...
      {/* Statistics Panel */}
      <StatisticsPanel performanceMetrics={performanceMetrics} learningSession={learningSession} selectedSegments={selectedSegments} />
