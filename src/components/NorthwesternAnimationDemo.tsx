/**
 * Northwestern Animation System Demo (TypeScript)
 *
...
import AnimatedNorthwesternPieChart, { ANIMATION_PHASES } from './AnimatedNorthwesternPieChart';
import NorthwesternInteractionSystem from './NorthwesternInteractionSystem';
import { AnimationPerformanceMonitor } from '../animations/NorthwesternAnimations';
...
interface AnimationMetricsData {
...
type AnimationSequence = 'educational' | 'clinical' | 'emergency';
type PerformanceMode = 'standard' | 'performance' | 'battery';

/**
 * Sample antibiotic data for demonstration
 */
const DEMO_ANTIBIOTICS: DemoAntibiotic[] = [
  {
    id: 'demo-vancomycin',
    name: 'Vancomycin',
    class: 'Glycopeptide',
...
interface DemoControlPanelProps {
  ...
  onAnimationSequenceChange: (sequence: AnimationSequence) => void;
...
const DemoControlPanel: FC<DemoControlPanelProps> = ({
  currentDemo,
  onDemoChange,
  emergencyMode,
  onEmergencyToggle,
  ...
  <div className="demo-controls bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <select
          value={currentDemo}
          onChange={(e) => onDemoChange(parseInt(e.target.value, 10))}
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => onAnimationSequenceChange(e.target.value as AnimationSequence)}
          className="w-full p-2 border border-gray-300 rounded text-sm"
        >
 ...
 */
interface AnimationMetricsProps {
  metrics: AnimationMetricsData;
  performanceReport: PerformanceReport | null;
}

const AnimationMetrics: FC<AnimationMetricsProps> = ({ metrics, performanceReport }) => (
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      <div>
        ...
        <div className="text-blue-900">
          {performanceReport?.averageFrameRate ? `${Math.round(performanceReport.averageFrameRate)} fps` : 'N/A'}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="text-blue-900">
          {performanceReport?.frameDropPercentage ? `${Math.round(performanceReport.frameDropPercentage)}%` : '0%'}
        </div>
      </div>
 ...
 * Main Demo Component
 */
interface NorthwesternAnimationDemoProps {
  onDemoComplete?: (metrics: PerformanceReport) => void;
  className?: string;
}

const NorthwesternAnimationDemo: FC<NorthwesternAnimationDemoProps> = ({
  onDemoComplete,
  className = ''
}) => {
  // Demo state
  ...
  const [animationSequence, setAnimationSequence] = useState<AnimationSequence>('educational');
  const [performanceMode, setPerformanceMode] = useState<PerformanceMode>('standard');
  const [reducedMotion, setReducedMotion] = useState<boolean>(false);

  // Animation tracking
  const [currentPhase, setCurrentPhase] = useState<string | null>(null);
  const [animationLog, setAnimationLog] = useState<AnimationLogEntry[]>([]);
  const [animationMetrics, setAnimationMetrics] = useState<AnimationMetricsData>({});
  ...
  useEffect(() => {
    ...
    return () => {
      if (performanceMonitorRef.current?.getReport) {
  ...
  const handleDemoChange = useCallback((demoIndex: number): void => {
  ...
  const handleAnimationStart = useCallback(
    (phase: string): void => {
      ...
      if (performanceMonitorRef.current?.recordAnimation) {
  ...
  const handleAnimationPhaseChange = useCallback(
    (phase: string): void => {
  ...
  const handleEmergencyToggle = useCallback((): void => {
    ...
    if (newEmergencyMode && performanceMonitorRef.current?.recordEmergencyOverride) {
  ...
  return (
    <div className={`northwestern-animation-demo ${className}`}>
      ...
      <div className="demo-header mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Northwestern Animation System Demo</h2>
        ...
        <div className="demo-chart-container">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-gray-900">{currentAntibiotic.name}</h3>
              {currentPhase && (
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${
                    currentPhase === ANIMATION_PHASES.EMERGENCY
                      ? 'bg-red-100 text-red-800'
                      : currentPhase === ANIMATION_PHASES.COMPLETE
          ...
          {/* Animation Metrics */}
          <div className="mt-4">
            <AnimationMetrics metrics={animationMetrics} performanceReport={performanceReport} />
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="animation-log max-h-96 overflow-y-auto space-y-2">
              {animationLog.slice(-20).reverse().map((entry, index) => (
                <div key={`${entry.timestamp.getTime()}-${index}`} className="log-entry bg-gray-50 rounded p-2 text-xs">
                  <div className="flex justify-between items-start">
                    <span
                      className={`font-medium ${
                        entry.event === 'emergency_mode_toggle'
                          ? 'text-red-600'
                          : entry.event === 'animation_complete'
                          ? 'text-green-600'
                          : 'text-blue-600'
                      }`}
                    >
                  ...
                  {entry.phase && <div className="text-gray-600 mt-1">Phase: {entry.phase}</div>}
                  {entry.antibiotic && <div className="text-gray-600 mt-1">Drug: {entry.antibiotic}</div>}
                  {entry.insight && <div className="text-gray-600 mt-1 italic">"{entry.insight}"</div>}
                </div>
              ))}

              {animationLog.length === 0 && (
                <div className="text-gray-500 text-center py-8">Interact with the pie chart to see animation events...</div>
              )}
            </div>
          </div>
