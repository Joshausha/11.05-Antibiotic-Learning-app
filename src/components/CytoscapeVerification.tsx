...
interface CytoscapeInstance {
  on: (event: string, selector: string, callback: (evt: any) => void) => void;
}

const CytoscapeVerification: FC = () => {
  ...
  // Simple test network
  const testElements: CytoscapeElement[] = [
  ...
  const stylesheet: CytoscapeStylesheet[] = [
    {
      selector: 'node',
      style: {
  ...
  const handleCytoscapeMount = (cy: CytoscapeInstance): void => {
    ...
    <div className="p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-4 p-4 bg-gray-50 rounded">
        ...
        <ul className="space-y-1">
          <li
            className={
              verificationStatus.imported ? 'text-green-600' : 'text-gray-400'
            }
          >
            {verificationStatus.imported ? '✓' : '○'} react-cytoscapejs imported
            successfully
          </li>
          <li
            className={
              verificationStatus.rendered ? 'text-green-600' : 'text-gray-400'
            }
          >
            {verificationStatus.rendered ? '✓' : '○'} Cytoscape component
            rendered
          </li>
          <li
            className={
              verificationStatus.interactive
                ? 'text-green-600'
                : 'text-gray-400'
            }
          >
            {verificationStatus.interactive ? '✓' : '○'} Interactive (click a
            node to verify)
          </li>
        </ul>
      </div>
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
        <p className="text-sm text-blue-800">
          <strong>Instructions:</strong> Click on any node in the graph below to
          verify interactivity. All three checkmarks should turn green for
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded text-sm">
        ...
        <ul className="space-y-1 text-gray-700">
          <li>
            • <strong>Package:</strong> react-cytoscapejs v2.0.0
          </li>
          <li>
            • <strong>Core Library:</strong> cytoscape v3.33.1
          </li>
          <li>
            • <strong>TypeScript Definitions:</strong> @types/cytoscape v3.21.9
          </li>
          <li>
            • <strong>React Version:</strong> 18.2.0 (compatible)
          </li>
          <li>
            • <strong>Webpack:</strong> 5.64.4 (compatible)
          </li>
        </ul>
      </div>
    </div>
