...
import {
  ...
  CoverageClinicalMap,
  TooltipPosition,
} from '../types/clinical-ui.types';

// Clinical context data for each Northwestern segment
const CLINICAL_DATABASE: ClinicalDatabase = {
  MRSA: {
  ...
  MSSA: {
    ...
    treatmentOptions: {
      ...
      alternative: ['Cephalexin', 'Clindamycin', 'Dicloxacillin'],
...
const COVERAGE_CLINICAL: CoverageClinicalMap = {
  0: {
    text: 'No Coverage',
 ...
 */
const EmergencyAlert: React.FC<EmergencyAlertProps> = ({ segmentKey, coverage, antibiotic }) => {
  const clinicalData = CLINICAL_DATABASE[segmentKey as keyof ClinicalDatabase];
 ...
 */
const TreatmentRecommendations: React.FC<TreatmentRecommendationsProps> = ({ segmentKey, coverage, educationLevel, emergencyMode }) => {
  const clinicalData = CLINICAL_DATABASE[segmentKey as keyof ClinicalDatabase];
 ...
 */
const ClinicalPearls: React.FC<ClinicalPearlsProps> = ({ segmentKey, educationLevel }) => {
  const clinicalData = CLINICAL_DATABASE[segmentKey as keyof ClinicalDatabase];
 ...
 */
const ClinicalTooltip: React.FC<ClinicalTooltipProps> = ({
  tooltipData,
  educationLevel = 'resident',
  emergencyMode = false,
  className = '',
  ...
  const clinicalData = CLINICAL_DATABASE[segment as keyof ClinicalDatabase];
