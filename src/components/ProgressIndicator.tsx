...
export const CircularProgress: React.FC<{
  ...
  const sizes: Record<string, { width: number; height: number; radius: number }> = {
...
export const LearningProgress: React.FC<{
  ...
  const getAchievementLevel = (prog: number) => {
    if (prog >= 90) return { text: "Expert Level", color: "text-green-600", icon: <Award className="w-5 h-5 text-green-600" /> };
    if (prog >= 75) return { text: "Advanced", color: "text-blue-600", icon: <Star className="w-5 h-5 text-blue-600" /> };
    if (prog >= 50) return { text: "Intermediate", color: "text-yellow-600", icon: <Target className="w-5 h-5 text-yellow-600" /> };
    if (prog >= 25) return { text: "Beginner", color: "text-orange-600", icon: <Activity className="w-5 h-5 text-orange-600" /> };
 ...
 */
export const MedicalTopicProgress: React.FC<{
  topicData?: {
